...
const getDefaultState = () => {
    return {
        ...
        PostAll: {},
        _Structure: {
            HelloworldPacketData: getStructure(HelloworldPacketData.fromPartial({})),
...
export default {
    ...
    actions: {
        init({ dispatch, rootGetters }) {
            console.log('Vuex module: dixitaniket.gitmony.helloworld initialized!');
        ...
        async QueryTimeoutPost({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryTimeoutPost', payload: { options: { all }, params: { ...key }, query } });
                return getters['getTimeoutPost']({ params: { ...key }, query }) ?? {};
            }
            catch (e) {
                throw new SpVuexError('QueryClient:QueryTimeoutPost', 'API Node Unavailable. Could not perform query: ' + e.message);
            }
        },
        async QueryTimeoutPostAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params: { ...key }, query = null }) {
            try {
                const queryClient = await initQueryClient(rootGetters);
                let value = (await queryClient.queryTimeoutPostAll(query)).data;
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await queryClient.queryTimeoutPostAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    value = mergeResults(value, next_values);
                }
                commit('QUERY', { query: 'TimeoutPostAll', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryTimeoutPostAll', payload: { options: { all }, params: { ...key }, query } });
                return getters['getTimeoutPostAll']({ params: { ...key }, query }) ?? {};
            }
            catch (e) {
                throw new SpVuexError('QueryClient:QueryTimeoutPostAll', 'API Node Unavailable. Could not perform query: ' + e.message);
            }
        },
        ...
        async QuerySendPostAll({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params: { ...key }, query = null }) {
            try {
                const queryClient = await initQueryClient(rootGetters);
                let value = (await queryClient.querySendPostAll(query)).data;
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await queryClient.querySendPostAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    value = mergeResults(value, next_values);
                }
                commit('QUERY', { query: 'SendPostAll', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySendPostAll', payload: { options: { all }, params: { ...key }, query } });
                return getters['getSendPostAll']({ params: { ...key }, query }) ?? {};
            }
            catch (e) {
                throw new SpVuexError('QueryClient:QuerySendPostAll', 'API Node Unavailable. Could not perform query: ' + e.message);
            }
        },
