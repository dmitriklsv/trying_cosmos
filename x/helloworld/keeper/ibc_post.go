...
func (k Keeper) OnRecvIbcPostPacket(ctx sdk.Context, packet channeltypes.Packet, data types.IbcPostPacketData) (packetAck types.IbcPostPacketAck, err error) {
	...
	ack_post_id := k.AppendPost(
		ctx,
		types.Post{
			Creator: packet.SourcePort + "-" + packet.SourceChannel + "-" + data.Title,
			Title:   data.GetTitle(),
			Content: data.GetContent(),
		},
	)
	packetAck.PostID = strconv.FormatUint(ack_post_id, 10)
...
func (k Keeper) OnAcknowledgementIbcPostPacket(ctx sdk.Context, packet channeltypes.Packet, data types.IbcPostPacketData, ack channeltypes.Acknowledgement) error {
	...
	case *channeltypes.Acknowledgement_Result:
		...
		// THIS IS THE UNMARSHALLING PROCESS WHEN ALL THE DATA THAT IS PRESENT HERE IS PRESENT IN THE POST ID
		...
		k.AppendSendPost(
			ctx,
			types.SendPost{
				Creator: packet.SourceChannel + "-" + data.Title,
				Title:   data.Title,
...
func (k Keeper) OnTimeoutIbcPostPacket(ctx sdk.Context, packet channeltypes.Packet, data types.IbcPostPacketData) error {
	...
	k.AppendTimeoutPost(
		ctx,
		types.TimeoutPost{
			Creator: packet.DestinationChannel + "-" + data.Title,
			Title:   data.Title,
			Chain:   packet.DestinationPort + "-" + packet.DestinationChannel,
		},
	)
